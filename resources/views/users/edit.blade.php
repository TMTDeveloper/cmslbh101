...
<div class="col-md-8">
  ...
  <form enctype="multipart/form-data" class="bg-white shadow-sm p-3" action="{{route('users.update', ['id'=>$user->id])}}" method="POST">
    ...
    <input value="{{$user->jabatan}}" class="form-control" placeholder="jabatan" type="text" name="jabatan" id="jabatan"/>
    <br>
    ...
    <input 
      type="checkbox"
      {{in_array("PROGRESS", json_decode($user->roles)) ? "checked" : ""}} 
      name="roles[]" 
      class="form-control {{$errors->first('roles') ? "is-invalid" : "" }}"
      id="PROGRESS" 
      value="PROGRESS"> 
      <label for="PROGRESS">Admin Transfer</label>

    <input 
      type="checkbox"
      {{in_array("HANDLING", json_decode($user->roles)) ? "checked" : ""}} 
      name="roles[]" 
      class="form-control {{$errors->first('roles') ? "is-invalid" : "" }}"
      id="HANDLING" 
      value="HANDLING"> 
      <label for="HANDLING">Manage Kasus</label>

    <input 
      type="checkbox"
      {{in_array("DATAINPUT", json_decode($user->roles)) ? "checked" : ""}} 
      name="roles[]" 
      class="form-control {{$errors->first('roles') ? "is-invalid" : "" }}"
      id="DATAINPUT" 
      value="DATAINPUT"> 
      <label for="DATAINPUT">Manage Data Person & Kelompok</label>
    ...
    <small class="text-muted">Kosongkan jika tidak ingin mengubah profil</small>
